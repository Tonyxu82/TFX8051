...
int before(){
	...
	if(enable_trace){
		trace = trace_new("socket", cur_trace);
	}
	return 0;
}
...
int main(int argc, char** argv){
	...
	*/
	...
	int len;
    char buf[4096];
	s = create_qfm_socket(0);
    if(s < 0) {
    	perror("Error: fail to create QFM socket!\n");
    	return 0;
    }

    sk = open_qfm_socket(s);
    if(!sk) {
    	perror("Error: fail to open QFM socket!\n");
    	return 0;
    }
