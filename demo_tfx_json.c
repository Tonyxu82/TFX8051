...
int before(){
	...
	if(enable_trace){
		trace = trace_new(cur_test->file, cur_trace);
	}

	if(enable_coverage){
		begin_coverage(cur_test->file, cur_core);
	}
...
int after(){
	...
	if(enable_coverage){
		end_coverage(cur_test->file, cur_core);
	}
...
int run_test(){
	...
	int match = 1;
	int i;
	for(i = 0; i<cur_test->expect_size[2]; i++){
...
int main(int argc, char** argv){
	...
	if(argc < 2){
		printf("Usage: ut_test test.json\n");
		return -1;
	}
	
	cur_test = read_test(argv[1]);
	if(!cur_test) return -1;

	before();

	if(load_fw(cur_core, cur_test->file)) return -1;
